...
li $a2, 2
jal wackySum
move $t0, $v0 # put return value in $t0 for safekeeping

li $v0, 10  # this line and next one just exit the program
syscall
...
wackySum:
		...
		sw $s1, 8($sp) # Store a0 on stack
		sw $s0, 4($sp) # Store a1 on stack
...
combineFour:
		add $t0, $a0, $a1 # Addes 'a' and 'b' to $t0
		add $t1, $a2, $a3 # Adds 'c' and 'd' to $t1
		add $v0, $t0, $t1 # Stores sum 
		andi $t0, $v0, 1 # Bitwise AND to check if $v0 if odd (t2 = 1) or if $v0 is even (t2 = 0)
		srlv $v0, $v0, $t0 # If $t2 = 1, we shift right by one bit. If $t2 = 0, we shift by 0 bits.
		jr $ra # Return
